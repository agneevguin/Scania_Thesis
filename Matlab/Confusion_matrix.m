...
%% Input Data
true_dir = '/home/scania/Scania/Agneev/True_Labels_blackened/9-Classes_9548-Images/Test/Labels/';
pred_dir = '/home/scania/Scania/Agneev/Download_Models/Agneev_Model_9_9548_Preliminary-test_With-pretrain_epoch_30.0/Segnet_Labels/';
im_true_all = [];
im_pred_all = [];
...
f_true = dir(strcat(true_dir,'*.bmp'));         % true_dir
for k = 1:size(f_true,1)
    file = strcat(true_dir,f_true(k).name);     % true_dir
    im_true{k} = imread(file);
    im_true{k} = imresize(im_true{k}, 0.5, 'nearest');      % 0.25 for full images, 0.5 for 9548 images
    im_true_all = vertcat(im_true_all, im_true{k});
end

f_pred = dir(strcat(pred_dir,'*.png'));           % pred_dir
...
[C_9_9548_With] = confusionmat(RGB_true_Vector,RGB_pred_Vector, 'order', order_9);
