...
class Visualization(VisualizationInterface):
    ...
    @override
    def process_data(self, input_id, input_data, output_data, file_name):
        ...
        #i = 100000
        #while os.path.exists("/home/scania/Scania/Agneev/Tmp/img_%s.png" % i):
        #    i += 1
        #input_image.save('/home/scania/Scania/Agneev/Tmp/img_%s.png' % i)
        #print ('Save PNG: ~/Scania/Agneev/Tmp/img_%s.png' % i)
        ...
        #i = 100000
        #while os.path.exists("/home/scania/Scania/Agneev/Tmp/seg_%s.png" % i):
        #    i += 1
        #seg_image.save('/home/scania/Scania/Agneev/Tmp/seg_%s.png' % i)
        #print ('Save PNG: ~/Scania/Agneev/Tmp/seg_%s.png' % i)
        ...
        background = input_image.convert("RGBA")
        overlay = seg_image.convert("RGBA")

        new_img = PIL.Image.alpha_composite(background, overlay)
        # new_img = PIL.Image.blend(background, overlay, 0.7)

        # *********************************************************
        #    FILES ARE REMOVED FROM TMP USING INFERENCE_AGNEEV
        #                       LINE 89
        # *********************************************************

        if file_name == 'Video_file':
            i = 100000
            while os.path.exists("/home/scania/Scania/Agneev/Tmp/new_%s.png" % i):
                i += 30
            new_img.save("/home/scania/Scania/Agneev/Tmp/new_%s.png" % i)
            logging.info('Saving: ~/Scania/Agneev/Tmp/new_%s.png' % i)
        else:
            new_img.save('/home/scania/Scania/Agneev/Tmp/'+file_name+'.png')
            logging.info('Saving: ~/Scania/Agneev/Tmp/'+file_name+'.png')
