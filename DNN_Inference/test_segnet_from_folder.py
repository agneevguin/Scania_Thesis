...
for file in os.listdir(args.data):
	...
	input_image_raw = caffe.io.load_image(args.data+file)
	...
	print os.path.split(args.model)[0]
	if not os.path.exists(os.path.split(args.model)[0]+'/Segnet_Labels'):
		os.mkdir(os.path.split(args.model)[0]+'/Segnet_Labels')
	tail = os.path.split(args.data+file)[1].split('.')[0]+'.bmp'

	scipy.misc.toimage(segmentation_rgb, cmin=0.0, cmax=255.0).save(os.path.split(args.model)[0]+'/Segnet_Labels/'+tail)
